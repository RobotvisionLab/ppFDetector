...
class Ganomaly(BaseModel):
    ...
    def __init__(self, opt, dataloader):
        ...
        self.l_adv = nn.SmoothL1Loss()
        self.l_con = nn.SmoothL1Loss()
        self.l_enc = nn.SmoothL1Loss()
        self.l_bce = nn.SmoothL1Loss()
        ...
        if self.opt.isTrain:
            self.netg.train()
            self.netd.train()
            self.optimizer_d = Adam_GCC(self.netd.parameters(), lr=self.opt.lr, betas=(self.opt.beta1, 0.999))
            self.optimizer_g = Adam_GCC(self.netg.parameters(), lr=self.opt.lr, betas=(self.opt.beta1, 0.999))
