...
class Encoder(nn.Module):
    ...
    def __init__(self, isize, nz, nc, ndf, ngpu, n_extra_layers=0, add_final_conv=True):
        ...
        main.add_module('initial-cbam-{0}'.format(ndf),
                        CBAM(channel = ndf))

        print('initial-conv-{0}-{1}'.format(nc, ndf))
        ...
        for t in range(n_extra_layers):
            ...
            main.add_module('extra-layers-{0}-{1}-relu'.format(t, cndf),
                            nn.LeakyReLU(0.2, inplace=True))
            main.add_module('extra-layers-{0}-{1}-cbam'.format(t, cndf),
                        CBAM(channel = ndf))

            print('extra-layers-{0}-{1}-conv'.format(t, cndf))
        ...
        while csize > 4:
            ...
            main.add_module('pyramid-{0}-relu'.format(out_feat),
                            nn.LeakyReLU(0.2, inplace=True))
            main.add_module('pyramid-{0}-cbam'.format(out_feat),
                        CBAM(channel = out_feat))

            print('pyramid-{0}-{1}-conv'.format(in_feat, out_feat))
        ...
        if add_final_conv:
            ...
            print('final-{0}-{1}-conv'.format(cndf, nz))
